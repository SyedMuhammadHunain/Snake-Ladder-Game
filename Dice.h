...
void UpdateDiceRolling(float deltaTime) {
    if (isRolling) {
        ...
        if (rollingTime < 1.0f) {
            ...
            snprintf(rollResultMessage, sizeof(rollResultMessage), "%s ROLLED A %d! NOW AT POSITION %d", 
    ...
    if (showSpecialMessage) {
        specialMessageTime -= deltaTime;
        if (specialMessageTime <= 0.0f) {
            showSpecialMessage = false; // Hide the message after the timeout
        }
    }
        if (gameOver) {
...
void DrawDice() {
    ...
    if (isRolling) {
        ...
        DrawTextEx(MyFont, rollResultMessage, (Vector2) {750, 330}, 18, 1.0f, BLACK);

         if (showSpecialMessage) {
                DrawTextEx(MyFont, specialMessage, (Vector2) {775, 300}, 18, 1.0f, RED);
