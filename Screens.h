...
void TITLE_SCREEN (){
    DrawTexture(titleBackground, 20, 20, WHITE); // LOGO
     DrawTextEx(MyFont, "WELCOME TO SNAKE & LADDER",(Vector2) {130, 200}, 60, 2.0f,  BLACK);
            ...
            if (IsKeyPressed(KEY_ENTER)) currentScreen = PLAYER_SELECTION;        
}
...
void INPUT_SCREEN (){
    ...
    DrawTextEx(MyFont, TextFormat("ENTER PLAYERS NAME %d :", nameInputActive + 1), (Vector2) {285, 180}, 50, 3.0f, BLACK);
            ...
            if (IsKeyPressed(KEY_ENTER)) {
                nameInputActive++;
                if (nameInputActive >= numPlayers) currentScreen = GAME;
            }
...
void GAME_SCREEN (float deltaTime){
            ...
            DrawDice();
            CheckDiceClick(); // Only roll dice if clicked on it
